...
class NotifBloc extends Bloc<ListPaginationEvent, DataListState<Notif>> {
  NotifBloc(this.repository) : super(const DataListState()) {
    on<ListPaginationEvent>(
      (event, emit) async {
        if (event is ResetSearchTermEvent) {
          emit(_resetSearching());
        } else if (event is FetchItemEvent) {
          DataListState<Notif> stateData = await fetchData(event.pageKey);
          emit(stateData);
  ...
  DataListState<Notif> _resetSearching() {
    return state.copyWith(
      status: LoadStatus.reset,
    );
  }

  Future<DataListState<Notif>> fetchData(int pageKey) async {
    ...
    try {
      ...
      List<Notif> newItems = data.map((x) => Notif.fromJson(x)).toList();
      final isLastPage = newItems.length < _pageSize;
      final nextPageKey = isLastPage ? null : pageKey + 1;

      List<Notif> itemListBefore;
      if (pageKey == 1) {
        itemListBefore = [];
      } else {
  ...
  DataListState<Notif> _resetPage() {
